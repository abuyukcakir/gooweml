 ...
 * @version $Revision: 1 $
 */
public class OzaBagMLISOUP extends OzaBagISOUP implements MultiLabelLearner, MultiTargetRegressor{
    ...
    public static Prediction combinePredictions(Prediction[] predictions, Instance inst) {
        ...
        for (int i = 0; i < result.numOutputAttributes(); i++) {
            votes.setValue(i, result.getVote(i, 1));
        }
        votes.normalize();

        //now write back the results..
        for (int i = 0; i < result.numOutputAttributes(); i++) {
            result.setVote(i, 1, votes.getValue(i));
            result.setVote(i, 0, 1 - votes.getValue(i));
        }
